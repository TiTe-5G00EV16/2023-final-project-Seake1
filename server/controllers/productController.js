...
//Checking out listed item. (showing data of it.)
...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    if (title.length < 3 || title.length > 50)
      errors.push("Title needs be at least 3 characters long and max 50; ");
    if (isNaN(Number(price))) errors.push("Price needs be a number; ");
    if (description.length < 3 || description.length > 1000)
      errors.push(
        "Description needs be at least 3 characters long and max 1000; "
      );
    if (/^[A-Za-z]+$/.test(city) == false)
      errors.push("City needs contains only english letters; ");
    if (!image.includes("image"))
      errors.push("The uploaded file needs be an image; ");
...
router.patch("/edit/:id", isAuth, async (req, res) => {
  let { title, price, description, city, category } = req.body;
  try {
    ...
    if (user._id.toString() !== product.seller.toString()) {
      errors.push("You have no permission! ");
    }
    if (title.length < 3 || title.length > 50)
      errors.push("Title needs be at least 3 characters long and max 50; ");
    if (isNaN(Number(price))) errors.push("Price needs be a number; ");
...
//When archieving item making it not active on home page.
...
// Archieving and activing item from archieved list.
...
// adding and deleting item to or from wishlist.
